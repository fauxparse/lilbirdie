...
interface WishlistsClientProps {
  initialData: SerializedWishlistSummary[];
}

function PrivacyIcon({ privacy }: { privacy: SerializedWishlistSummary["privacy"] }) {
  switch (privacy) {
    case "PUBLIC":
...
function PrivacyLabel({ privacy }: { privacy: SerializedWishlistSummary["privacy"] }) {
  switch (privacy) {
    case "PUBLIC":
      return "Public";
    case "FRIENDS_ONLY":
...
export function WishlistsClient({ initialData }: WishlistsClientProps) {
  const {
    data: wishlists,
    isLoading,
    error,
  } = useQuery<SerializedWishlistSummary[]>({
    queryKey: ["wishlists"],
    queryFn: async () => {
